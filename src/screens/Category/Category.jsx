...
const Category = () => {
  ...
  const submitData =async (e)=>{
    e.preventDefault();
    const { name1,email,mobile,experince,file}=data;
  
    
      const res = fetch('https://bookyourtravel-ac401-default-rtdb.firebaseio.com/customerReview.json',
    ...
    body:JSON.stringify({
      name1, email, mobile, experince, file,
    })
  }
  )
...
if(res){
  ...
  return (
    <div>
      ...
      >
        <Input name= "name1" value={data.name1} onChange={postData} />
      </Form.Item>
      <Form.Item
        name={['user', 'email']}
        label="Email"
      ...
      >
        <Input name= "email" value={data.email} onChange={postData}/>
      </Form.Item>
      <Form.Item
        name={['user', 'phone']}
        label="Mobile_No"
      ...
      >
        <Input name= "mobile" value={data.mobile} onChange={postData} />
      </Form.Item>
     
      <Form.Item name={['user', 'introduction']} label="Experience">
        <Input.TextArea name= "experince" value={data.experince} onChange={postData} />
      </Form.Item>
      <Form.Item name={['user', 'website']} label="Share Photos">
        <Input type="file" style={{border: "none"}} name= "file" value={data.file} onChange={postData}/>
      </Form.Item>
      <Form.Item wrapperCol={{ ...layout.wrapperCol, offset: 8 }}>
        <Button type="primary" block htmlType="submit" onClick={submitData}>
          Submit
        </Button>
      </Form.Item>
    </Form>
